...
export class PostgresDriver implements DriverContract {
  ...
  async beginTransaction(): Promise<Transaction> {
    return new Transaction(new PostgresDriver(await this.client.transaction()))
  }
  ...
  buildDistinct(...columns: string[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.distinct(...columns)

    return this
  }

  buildGroupBy(...columns: string[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.groupBy(...columns)

    return this
  }

  buildGroupByRaw(raw: string, queryValues?: any[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.groupByRaw(raw, queryValues)

    return this
  }

  buildHaving(column: string, operator: string, value: any): PostgresDriver {
  ...
  buildJoinRaw(raw: string, queryValues?: any[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.joinRaw(raw, queryValues)

    return this
  }

  buildLimit(number: number): PostgresDriver {
    this.queryBuilder = this.queryBuilder.limit(number)

    return this
  }

  buildSkip(number: number): PostgresDriver {
  ...
  buildOrderBy(column: string, direction?: 'asc' | 'desc'): PostgresDriver {
    this.queryBuilder = this.queryBuilder.orderBy(column, direction)

    return this
  }

  buildOrderByRaw(raw: string, queryValues?: any[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.orderByRaw(raw, queryValues)

    return this
  }

  buildSelect(...columns: string[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.select(...columns)

    return this
  }

  buildTable(tableName: string): PostgresDriver {
  ...
  buildWhereBetween(columnName: string, values: [any, any]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereBetween(columnName, values)

    return this
  }

  buildWhereExists(callback: any): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereExists(callback)

    return this
  }

  buildWhereIn(columnName: string, values: any[]): PostgresDriver {
  ...
  buildWhereNotBetween(columnName: string, values: [any, any]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereNotBetween(columnName, values)

    return this
  }

  buildWhereNotExists(callback: any): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereNotExists(callback)

    return this
  }

  buildWhereNotIn(columnName: string, values: any[]): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereNotIn(columnName, values)

    return this
  }

  buildWhereNull(columnName: string): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereNull(columnName)

    return this
  }

  buildWhereNotNull(columnName: string): PostgresDriver {
    this.queryBuilder = this.queryBuilder.whereNotNull(columnName)

    return this
  }

  buildWhereRaw(raw: string, queryValues?: any[]): PostgresDriver {
